...
$headers.name                       = "Ruby Rally Changeset Cleanup"
...
source_artifact_query = RallyAPI::RallyQuery.new()
source_artifact_query.type = query_type
source_artifact_query.fetch = "ObjectID,FormattedID,Name,Changesets,Author,Revision,Message,CommitTimeStamp"
source_artifact_query.order = "FormattedID Asc"
source_artifact_query.project_scope_down = $project_scope_down
source_artifact_query.query_string = query_string

artifact_query_results = @rally.find(source_artifact_query)
...
puts "Found #{number_of_artifacts} artifacts for possible changeset cleanup."

artifact_changesets = found_artifact["Changesets"]

if !artifact_changesets.nil? then
  puts "List of changesets for #{artifact_formatted_id}..."
  changeset_number = 1
  artifact_changesets.each do | this_changeset |
    puts "#{changeset_number}. Revision: #{this_changeset.Revision}; Author: #{this_changeset.Author}, Message: #{this_changeset.Message}; CommitTimeStamp: #{this_changeset.CommitTimeStamp}"
    changeset_number += 1
  end

  # Ask user for Number of Changeset to delete
  puts "Please enter a changeset number to delete."
  puts
  changeset_to_delete = [(print 'Enter Number: '), gets.rstrip][1].upcase

  # Perform error checking
  changeset_number = artifact_formatted_id[/\d+/]

  if changeset_number.nil? or changeset_number.to_i > artifact_changesets.length then
    puts "Invalid number. Please specify a valid number. Exiting."
    exit
  end

  changeset_number = changeset_number.to_i
  puts "PERMANENTLY delete changeset: #{artifact_changesets[changeset_number-1].Revision}; ?"

  really_delete = [(print '[N/y]: '), gets.rstrip][1].upcase

  if really_delete == "Y" then
  begin
    delete_result = @rally.delete(artifact_changesets[changeset_number-1]._ref)
    puts "DELETED changeset #{artifact_changesets[changeset_number-1].Revision}: #{artifact_changesets[changeset_number-1].Author}"
    rescue => ex
      puts "Error occurred trying to delete: #{artifact_changesets[changeset_number-1].Revision}: #{artifact_changesets[changeset_number-1].Author}"
      puts ex
    end
  else
    puts "Did NOT delete #{artifact_changesets[changeset_number-1].Revision}: #{artifact_changesets[changeset_number-1].Author}"
  end

  puts "Complete!"

else
  puts "Artifact: #{artifact_formatted_id} has no Changesets"
end